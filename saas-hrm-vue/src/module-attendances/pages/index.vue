<template>
  <div class="dashboard-container">
    <div class="app-container">
      <AttendancesPageTool :monthOfReport="monthOfReport"
                           @dataList="dataList"
                           :formData="formData"
                           :month="month"
                           :yearMonth="yearMonth" />
          ...
          <div style=" width: 3000px;">
            <table border="0"
              ...
              <tr>
                ...
                <th width="200">部门</th>
                <th width="100">手机</th>
                <th v-for="(it, ind) in monthOfReport"
                    :key="ind"
                    width="110">{{attendInfo.month}}/{{ind+1}}</th>
              </tr>
              <tr v-for="(item, index) in baseData"
                ...
                <td v-for="(it,ind) in item.attendanceRecord"
                  ...
                  <span v-if="it.adtStatu===23">休息</span>
                </td>
              </tr>
            </table>
          </div>
          <div class="page-list">
            <div class="pagination">
              <el-pagination @size-change="handleSizeChange"
                             ...
                             :total="Number(attendInfo.counts)"></el-pagination>
            </div>
            <!-- end -->
          </div>
        </div>
        <div v-else
             class="textInfo">暂无数据</div>
        <el-dialog :visible.sync="centerDialogVisible"
                   width="30%"
                   center>
          <span slot="title"
                style="color:#fff;">{{attendInfo.name}} {{attendInfo.month}}/{{attendInfo.getDate}}（实际工作日考勤方案）</span>
          <div class="attenInfo">
            <p class="colRed">注：统计考勤时，异常状态优先正常状态^_^</p>
            <p class="check">
              <el-radio-group v-model="modifyData.adtStatu">
                <el-radio v-for="item in stateData.holidayType"
                          :label="item.id"
                          :key="item.id"
                          :value="item.value">{{item.value}}</el-radio>
              </el-radio-group>
            </p>
          </div>
          <span slot="footer"
